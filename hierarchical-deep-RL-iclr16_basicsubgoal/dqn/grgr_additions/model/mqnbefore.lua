...
local MQN, parent = torch.class('MQNBefore', 'Net')
function MQN:build_model(args)
    ...
    subgoal_features = nn.View(-1, 11, 5)(subgoal_features)
    local history = nn.Narrow(2, 1, T-1)(cnn_features)
    history = nn.JoinTable(3)({history, subgoal_features})
...
function MQN:build_subgoal(args, input)
    ...
    local subgoal_linear2 = nn.Linear(args.subgoal_nhid, 55)(subgoal_ReLu1)
    nngraph.annotateNodes()
